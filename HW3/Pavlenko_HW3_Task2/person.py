class Person:
    ...
    def __init__(self, surname=None, name=None, age=None, *args, **kwargs):
        if not isinstance(surname, str):
            raise TypeError('erewrewr')
        if not surname:
            raise ValueError('Surname isn\'t correct')
        if not name:
            raise ValueError('Name isn\'t correct')
        if age not in range(16, 100):
           raise ValueError('Age isn\'t correct')
...
class Group:
    ...
    def add_to_group(self, student):
        if not isinstance(student, Student):
            raise TypeError('Is not Student')
...
stud1 = Student('123', 'trytry', 20, 95)
